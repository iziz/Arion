...
7. `vision-detection` and `vision-tracking`
   ...
   - Tracker output includes run-level and segment-level diagnostics for run provenance, media fingerprints, runtime sampling, frame coverage, sampled frame audits, box and track distributions, OCR and face decision audits, dependency versions, model/gallery/calibration fingerprints, and homography/team-direction calibration status.
   - Asset-level `rawMatchProfile` summarizes what can be known from an unknown source recording, separating technical metadata, observed cues, tracking readiness, identity readiness, event readiness, confirmed source context, and explicit limitations.
...
The resolver intentionally does not claim stable visual identity from generic face recognition. Current tracking can add heuristic kit-color cluster evidence, crop-based jersey number candidates, screen-relative pitch-zone/lane occupancy, homography-projected track coordinates, and team attacking-direction calibration to track candidates. Roster-backed face embedding candidates are produced only when `FACE_IDENTITY_ENABLED`, an ONNX embedding model, and a generated player gallery are configured; otherwise the face evidence path stays inactive. Helmet assignment, stronger ReID, contact, and calibrated tracking remain candidate evidence unless context, clock, roster/participant evidence, and track evidence agree.
...
- Sports player identity is candidate-first. The current resolver can use match/game context, roster or lineup windows, nflverse play metadata, OCR/ASR/VLM text, MOT track IDs, heuristic kit-color track clusters, crop jersey OCR candidates, screen-relative or homography-projected pitch-zone/lane occupancy, position metadata, team attacking-direction calibration, and roster-backed face embedding candidates. Helmet/stronger ReID/contact evidence hooks remain available, but the system does not implement face-first stable identity without a configured face model and player gallery.

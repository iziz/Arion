...
export function segmentSearchText(segment: TimelineSegment) {
  ...
  const tagText = segment.tags.join(" ");
  if (!text) return [segment.summary, segment.transcript, tagText, domainText, vlmText].filter(Boolean).join(" ");
  return [segment.summary, text.speech, ...text.subtitles, ...text.screenText, ...text.overlays, tagText, domainText, vlmText].filter(Boolean).join(" ");
}
